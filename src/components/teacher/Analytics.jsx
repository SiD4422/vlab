...
export function Analytics({
  ...
  // avgViva is a string like "2.5" (on 0-3 viva scale)
  // avgTeacher is a string like "72.5%" (normalised to percent)
  const vivaBarPct = avgViva === '-' ? 0 : Math.min(100, (parseFloat(avgViva) / 3) * 100);
  const teacherBarPct = avgTeacher === '-' ? 0 : parseFloat(avgTeacher);

  const evadersSet = new Set();
  const evasionCounts = {};
  submissions.forEach(s => {
    if (s.status === 'grading_failed') {
      evasionCounts[s.studentUid] = (evasionCounts[s.studentUid] || 0) + 1;
      if (evasionCounts[s.studentUid] > 1) {
        evadersSet.add(s.studentUid);
      }
    }
  });
  const serialEvadersCount = evadersSet.size;
              ...
              <div>
                ...
                <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:24,marginBottom:40 }}>
                  {[
                    ...
                    { label:'Pending Review', value:submissions.filter(s=>s.status !== 'teacher_reviewed').length,          icon:Clock,         color:'239,68,68' },
                    { label:'Serial Evaders', value:serialEvadersCount,                                                     icon:AlertTriangle, color:'220,38,38' },
                  ].map((stat,i)=>(
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 32, marginBottom: 40 }}>
                  ...
                  <div style={{ background: '#fff', borderRadius: 24, padding: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
                     ...
                     <div style={{ marginBottom: 20 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 14, fontWeight: 700, color: '#64748b' }}>
                           <span>AI Viva Score (avg)</span>
                           <span style={{ color: '#10b981' }}>{avgViva} / 3</span>
                        </div>
                        <div style={{ height: 12, background: '#f1f5f9', borderRadius: 6, overflow: 'hidden' }}>
                           <div style={{ height: '100%', width: `${vivaBarPct}%`, background: 'linear-gradient(90deg, #10b981, #34d399)', borderRadius: 6 }} />
                        </div>
                     </div>
                     
                     <div style={{ marginBottom: 20 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 14, fontWeight: 700, color: '#64748b' }}>
                           <span>Teacher Grading (avg)</span>
                           <span style={{ color: '#8b5cf6' }}>{avgTeacher}</span>
                        </div>
                        <div style={{ height: 12, background: '#f1f5f9', borderRadius: 6, overflow: 'hidden' }}>
                           <div style={{ height: '100%', width: `${teacherBarPct}%`, background: 'linear-gradient(90deg, #6366f1, #8b5cf6)', borderRadius: 6 }} />
                        </div>
                     </div>
                  ...
                  <div style={{ background: '#fff', borderRadius: 24, padding: 32, boxShadow: '0 4px 20px rgba(0,0,0,0.03)', border: '1px solid #f1f5f9' }}>
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        {[...submissions]
                          // Normalise to 0-100 for fair comparison across old (/10) and new (/100) subs
                          .sort((a,b) => {
                            const sa = a.gradingVersion === 1 ? (a.teacherScore||0) : (a.teacherScore||0)*10;
                            const sb = b.gradingVersion === 1 ? (b.teacherScore||0) : (b.teacherScore||0)*10;
                            return sb - sa;
                          })
                          .slice(0,3)
                          .map((sub, i) => {
                            const score = sub.gradingVersion === 1 ? sub.teacherScore : (sub.teacherScore != null ? sub.teacherScore * 10 : null);
                            return (
                             <div key={sub.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                ...
                                <div style={{ fontSize: 14, fontWeight: 800, color: '#10b981' }}>{score != null ? `${score}/100` : '—'}</div>
                             </div>
                            );
                          })}
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
                  {submissions.slice(0,5).map((sub,i)=>{
                    const isReviewed = sub.status === 'teacher_reviewed';
                    const isAutoGraded = sub.status === 'auto_graded';
                    const scoreLabel = sub.teacherScore != null
                      ? `${sub.teacherScore}/${sub.gradingVersion === 1 ? '100' : '10'}`
                      : null;
                    const borderColor = isReviewed ? '#8b5cf6' : isAutoGraded ? '#10b981' : '#f59e0b';
                    return (
                    <div key={sub.id} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 24px',background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',boxShadow:'0 2px 8px rgba(0,0,0,0.02)',borderLeft:`4px solid ${borderColor}`,transition:'transform 0.2s, boxShadow 0.2s' }} onMouseEnter={e=>{e.currentTarget.style.transform='translateX(4px)'; e.currentTarget.style.boxShadow='0 6px 16px rgba(0,0,0,0.06)';}} onMouseLeave={e=>{e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow='0 2px 8px rgba(0,0,0,0.02)';}}>
                      ...
                      <div style={{ display:'flex',alignItems:'center',gap:16 }}>
                        {scoreLabel?(
                          <span style={{ background: isReviewed ? 'linear-gradient(135deg,#8b5cf6,#a855f7)' : 'linear-gradient(135deg,#10b981,#34d399)',color:'#fff',fontSize:12,fontWeight:800,padding:'5px 14px',borderRadius:999,boxShadow:'0 2px 8px rgba(16,185,129,0.3)' }}>
                            {isReviewed ? '✏️ ' : '✅ '}{scoreLabel}
                          </span>
                        ):(
                          <button onClick={()=>{ setSelectedSubmission(sub); setReportTab('overview'); setActiveNav('submissions'); }} style={{ background:'linear-gradient(135deg,#f59e0b,#f97316)',color:'#fff',border:'none',fontSize:12,fontWeight:800,padding:'6px 16px',borderRadius:999,cursor:'pointer',boxShadow:'0 2px 8px rgba(245,158,11,0.3)',transition:'transform 0.2s' }} onMouseEnter={e=>e.target.style.transform='scale(1.05)'} onMouseLeave={e=>e.target.style.transform='none'}>Review</button>
                        )}
